...
parser.add_argument(
    "--exit-mode",
    action="store",
    help="If 'soft', this will exit with a code 1 on error. If it is 'hard', it will instead just raise a "
    "traceback on error."
...
def main():
    ...
    for record in records:
        ...
        if record["id"] == args.record_name or record["name"] == args.record_name:
            record_id = record["id"]
            assert edit_dns_record(zone_id, record_id, content=...), "Failed to edit a record."
            break
    else:
        assert create_dns_record(
            ...
            content=...,
            proxied=args.create_is_proxied,
